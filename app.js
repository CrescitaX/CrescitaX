...
console.log('CrescitaX v1.0');

const APP_CONFIG = {
  storageKey: 'crescitax_data_v1',
  ...
  levels: [
    ...
    { threshold: 500, name: 'Costante', badge: '🔥', color: '#f97316' },
    { threshold: 1000, name: 'Guru', badge: '👑', color: '#8b5cf6' }
  ],
  pointsSystem: {
    habitCompletion: 10,
    streak7Days: 50,
...
if (completeBtn) {
    ...
    <div class="habit-action-icon" id="delete-${habit.id}" title="Elimina abitudine">🗑️</div>
                </div>
...
/**
 * Elimina l'abitudine indicata e aggiorna UI + dati
 */
deleteHabit(habitId) {
    const data = loadAppData();
    ...
    // Rimuovi
    const [removed] = data.habits.splice(index, 1);

    // Rimuovi eventuali chart salvati
    if (this.chartInstances && this.chartInstances[habitId]) {
        this.chartInstances[habitId].destroy();
        delete this.chartInstances[habitId];
    }
